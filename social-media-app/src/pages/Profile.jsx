...
const CoolProfilePage = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                console.log('Fetched Profile:', profileData); // Log fetched data
    ...
    useEffect(() => {
        console.log('Profile updated:', profile); // Log profile info whenever it updates
    }, [profile]);
    ...
    useEffect(() => {
        const fetchUserPosts = async () => {
            try {
                ...
                setPosts(userPosts); // Update state with fetched posts
            } catch (error) {
    ...
    useEffect(() => {
        console.log("Posts updated:", posts); // Log posts info whenever it updates
    }, [posts]);
            ...
            <Card className="fixed top-8 left-20 w-80 h-[28rem] p-6 space-y-6 z-10">
                <div className="flex flex-col items-center space-y-4">
                    <Avatar className="w-32 h-32 relative overflow-hidden rounded-full transition-transform duration-300 ease-in-out hover:scale-110 hover:rotate-3 hover:shadow-xl hover:shadow-gray-500/50">
                        <AvatarImage src={profile.profilePic} alt={profile.username || "Profile Picture"} className="w-full h-full object-cover transition-transform duration-300 ease-in-out hover:scale-110 hover:rotate-3 hover:shadow-xl hover:shadow-gray-500/50" />
